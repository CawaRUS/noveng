...
public:
    void execute(NovelEngine* eng, const std::vector<std::string>& args) override {
        ...
        if (!isValidIdentifier(args[0])) {
            Logger::getInstance().warn("Command: dec refused invalid variable name: " + args[0]);
            return;
        }
        ...
        if (args.size() >= 2 && !args[1].empty()) {
            try {
                ...
                Logger::getInstance().warn("Command: dec failed to evaluate delta '" + args[1] + "': " + e.what());
                return;
            }
        }
        ...
        Logger::getInstance().debug("Command: dec set " + args[0] + " = " + variantToString(var));
    }
};
REGISTER_COMMAND(CmdDec, "dec")
